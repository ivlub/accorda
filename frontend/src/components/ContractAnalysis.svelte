<script lang="ts">
  ...
  // Okay so the steps are: first is pick type of law. then file upload. then analysis. 1.2.3. prettyy easy.
  ...
  const API_BASE_URL = ''; // Use relative path for API calls. This is what was messing me up when doing devops lol
  ...
  // Helper function to calculate status counts for GENERAL requirements (So like the nice, missing, review needed bubble counts)
  ...
  // This is the function that exports the analysis to a PDF. I don't like it. I don't like work with documents.
  async function exportToPdf() {
    ...
    try {
      ...
      const addFooter = (doc: jsPDF, pageNum: number) => {
          ...
          // Add Confidentiality Notice. Lawyers are funny.
